	...
	for d.More() {
		...
		case interface{ Unwrap() []error }:
			...
			for _, innerErr := range innerErrs {
				messages = append(messages, processError(innerErr, emp))
			}
...
func ValidateEmployee(e Employee) error {
	var errs []error
	if len(e.ID) == 0 {
		errs = append(errs, EmptyFieldErr{"ID"})
	}
	if !validID.MatchString(e.ID) {
		errs = append(errs, ErrInvalidId)
	}
	if len(e.FirstName) == 0 {
		errs = append(errs, EmptyFieldErr{"FirstName"})
	}
	if len(e.LastName) == 0 {
		errs = append(errs, EmptyFieldErr{"LastName"})
	}
	if len(e.Title) == 0 {
		errs = append(errs, EmptyFieldErr{"Title"})
	}
	switch len(errs) {
	case 0:
		return nil
